...
class Statiz:
    ...
    @property
    def hit_basic(self):
        self.driver.get(self.record_site(year)['타격기본'])
    ...
    @property
    def hit_expand(self):
        self.driver.get(self.record_site(year)['타격확장'])
        hit_expand_df_list = []
        while True:
            page = self.driver.page_source
            df = pd.read_html(page)[1]
    ...
    def pitch_basic(self):
        self.driver.get(self.record_site(year)['투수기본'])
        pitch_basic_df_list = []
        while True:
            page = self.driver.page_source
            df = pd.read_html(page)[1]
    ...
    def pitch_expand(self):
        self.driver.get(self.record_site(year)['투수확장'])
    ...
    def pitch_speed(self):
        self.driver.get(self.record_site(year)['투수구속'])
        pitch_speed_df_list = []
        while True:
            page = self.driver.page_source
            df = pd.read_html(page)[1]
